...
func TestMigrate_AdditionalVolumes_PreservesSameSourceDestination(t *testing.T) {
	...
	mock.InspectContainerResult = &containermanager.InspectResult{
		...
		Mounts: []containermanager.MountInfo{
			...
			// Home and XDG_RUNTIME_DIR mounts are regenerated by create from
			// the user environment and must not be treated as user volumes.
			{Source: "/home/testuser", Destination: "/home/testuser"},
			{Source: xdgRuntimeDir, Destination: xdgRuntimeDir},
			// User volumes with identical source and destination are
			// legitimate and must be preserved (regression for /opt:/opt).
			{Source: "/opt", Destination: "/opt"},
			{Source: "/data", Destination: "/data", Options: "ro"},
		},
	}
	...
	// Same-path user volumes must survive the filter.
	for _, want := range []string{"/opt:/opt", "/data:/data:ro"} {
		found := false
		for _, vol := range createOpts.AdditionalVolumes {
			if vol == want {
				found = true
		...
		if !found {
			t.Errorf("expected additional volume %q in %v", want, createOpts.AdditionalVolumes)
		}
	}

	// Home and XDG_RUNTIME_DIR mounts must be dropped, not passed through
	// as user volumes.
	for _, unwanted := range []string{"/home/testuser:/home/testuser", xdgRuntimeDir + ":" + xdgRuntimeDir} {
		for _, vol := range createOpts.AdditionalVolumes {
			if vol == unwanted {
				t.Errorf("expected mount %q to be dropped, got %v", unwanted, createOpts.AdditionalVolumes)
			}
		}
	}
}
